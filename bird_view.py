...
class Bird_view():
    def __init__(self):
        ...
        self.folder_path = "eleventh"  # Thirteen or eleventh
        self.main()

    def main(self):
        names = settings.camera_names
        # folder_path = "thirteen"  # ------------------------------------------------------------------------------
        images = [os.path.join(os.getcwd(), "data/", self.folder_path + "/undistortion/" + name + ".png") for name in self.names]
        yamls = [os.path.join(os.getcwd(), "data/", self.folder_path + "/yaml/" + name + ".yaml") for name in self.names]
        ...
        cv2.imwrite("data/" + self.folder_path + "/Resulttla.png", birdview.image)
